...
/**
 * Greeedy search: select token with maximum log proabiblity
 * Greedy sampling algorithm, pick the token index with the largest logit value
 * @param logits Tensor sotring raw logit scores of all vocabulary tokens
 ...
 * Core autoregressive inferece loop to generate token sequence step by step
 * @param model Pointer to initialized LLaMA model struct
 * @param cache Pointer to KVCache for attention state reuse
    ...
    u32* out_tokens, u32 max_gen_tokens)
{
    ...
    // Hidden state tensor storing embedding vector of curren token
    ...
    // Release all dinamically allocated temporary tensors to prevent memory leaks
    tensor_free(hidden);
    tensor_free(q);
    tensor_free(k);
    tensor_free(v);
    ...
    tensor_free(logits);
    // Return total length of combined prompt + generate token sequence
    return total_tokens;
}