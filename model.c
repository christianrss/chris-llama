...
void rope(f32* q, f32* k, u32 pos, u32 dim, u32 head_dim)
{
    ...
    {
        // Calculate relativeindex inside single attention head
        ...
        f32 q0 = q[i];
        f32 q1 = q[i + 1];
        q[i]     = q0 * cost_t - q1 * sin_t;
        q[i + 1] = q0 * sin_t  - q1 * cost_t;
        ...
        f32 k0 = k[i];
        f32 k1 = k[i + 1];
        k[i]     = k0 * cost_t - k1 * sin_t;
        k[i + 1] = k0 * sin_t  + k1 * cost_t;
    }
}
...
void swiglu(f32* out, const f32* gate, const f32* up, u32 dim)
{
    for (u32 i = 0; i < dim; i++)
    {
        ...
        // Element-wise product between activate gate and up vector
        out[i] = silu * up[i];
    }
}
...
int llama_model_init(LLaMAModel* model, LLaMAConfig* cfg)
{
    ...
    // Alocate array storage for all decoder layers
