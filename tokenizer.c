...
u32 text_to_tokens(const char* text, u32* tokens, u32 max_tokens)
{
    ...
    char buf[64];
    // Current write position index inside temporary word buffre
    u32 buf_idx = 0;
    ...
    for (u32 i = 0; text[i] != '\0'; i++)
    {
        ...
        if (text[i] == ' ' || text[i] == '\n')
        {
            // Skip empty buffer if no chracters were collected for current word
            if (buf_idx == 0) continue;
            // Add null terminator to mark end of assembled word string
            buf[buf_idx] = '\0';
            // Reset buffer index for next word collction
            buf_idx = 0;
 ...
 * @param token Input integer tokenID to decode
 * @param out_str Output char bufer for decoded word
...
void token_to_text(u32 token, char* out_str, u32 str_len)
{
    ...
    // Exit early iftoken ID exceeds vocabulary range (invalid token)
    if (token >= MAX_TOKEN_VOCAB)
        return;
    // Copy matched vocabulary word into output buffer, reserve space for null
