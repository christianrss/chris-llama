...
typedef struct {
    ...
    u32 n_layers;   // Total transformers decoder layers (22)
    u32 n_heads;    // Parallel attention hads count (32)
...
// Top-level container storing coplete TinyLlama model
typedef struct {
    LLaMAConfig cfg;
 ...
 * @param dim Hidden vector dimenson size
 */
...
/**
 * @brief Initialize empty LLaMA & allocate all tensor memory buffers
